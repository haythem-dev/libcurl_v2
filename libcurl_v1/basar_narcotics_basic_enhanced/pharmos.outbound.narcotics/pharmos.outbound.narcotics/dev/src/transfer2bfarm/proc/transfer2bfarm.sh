#! /usr/bin/ksh
 ...
 #Purpose:           Starts, stops and status the binary transfertobfarm.          
 ###############################################################################################
 ...
 readonly BINARY=transfer2bfarm.bin
 ...
 status="";
 typeset -i rc=0;
 typeset -i crosscheck=0;
 typeset -i transfer=0;
 typeset -i archive=0;
 contributionvoucherdir="";
 ...
 readonly CROSSCHECK_OPTION="-crosscheck";
 readonly TRANSFER_OPTION="-transfer";
 readonly ARCHIVE_OPTION="-archive";
 readonly CONTRIBUTIONVOUCHERDIR_OPTION="-contributionvoucherdir";
 ...
 function print_local_var
 {
	 ...
	 printf "%-30s%-90s\n" "\$CROSSCHECK_OPTION:" ">$CROSSCHECK_OPTION<"
	 printf "%-30s%-90s\n" "\$TRANSFER_OPTION:" ">$TRANSFER_OPTION<"
	 printf "%-30s%-90s\n" "\$ARCHIVE_OPTION:" ">$ARCHIVE_OPTION<"
	 printf "%-30s%-90s\n" "\$CONTRIBUTIONVOUCHERDIR_OPTION:" ">$CONTRIBUTIONVOUCHERDIR_OPTION<"
	 ...
	 printf "%-30s%-90s\n" "\$transfer:" ">$transfer<"
	 printf "%-30s%-90s\n" "\$archive:" ">$archive<"
	 printf "%-30s%-90s\n" "\$contributionvoucherdir:" ">$contributionvoucherdir<"
 	 echo "--------------------------"
 	 echo "EXIT $FUNCNAME"
 	 echo "--------------------------"
 }
 ...
 function eval_parameters
 {
 	 ...
 	 do
 		 ...
 		 #read in parameters
 		 if [[ "$1" == "$CROSSCHECK_OPTION" ]]; then
 			crosscheck=1;
			shift;
			continue;
 		 fi
 		 if [[ "$1" == "$TRANSFER_OPTION" ]]; then
 			transfer=1;
			shift;
			continue;
 		 fi
 		 if [[ "$1" == "$ARCHIVE_OPTION" ]]; then
 			archive=1;
			shift;
			continue;
 		 fi
 		 if [[ "$1" == "$CONTRIBUTIONVOUCHERDIR_OPTION" ]]; then
 			 shift;
 			 if (( $# > 0 )) ; then
 				 contributionvoucherdir=$1;
 				 shift;
 				 continue;
 			 else 
 				 break;
 ...
 function assembling_process
 {
	 ...
	 if (( ${crosscheck} == 1 )); then
 		 _parameters="${_parameters} -crosscheck"
	 fi
	 if (( ${transfer} == 1 )); then
 		 _parameters="${_parameters} -transfer"
	 fi
	 if (( ${archive} == 1 )); then
 		 _parameters="${_parameters} -archive"
	 fi

	 if [[ ${contributionvoucherdir} != "" ]]; then
 		 _parameters="${_parameters} -contributionvoucherdir ${contributionvoucherdir}"
	 fi
 ...
 ###############################################################################################
 . $(dirname $0)/transfer2bfarm.option
