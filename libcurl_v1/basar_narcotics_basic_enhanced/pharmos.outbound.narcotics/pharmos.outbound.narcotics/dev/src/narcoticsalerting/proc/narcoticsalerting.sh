#! /usr/bin/ksh
 ...
 readonly BINARY=narcoticsalerting.bin
 ...
 function assembling_process
 {
	 ...
	 if [[ "${startdate}${enddate}${contributionvoucherdir}" != "" ]]; then
 		 _parameters="${_parameters} ${startdate} ${enddate} ${contributionvoucherdir}"
	 fi
 ...
 ###############################################################################################
 . $(dirname $0)/narcoticsalerting.option
 ...
 if [[ "$start" == "YES" ]]; then
	 ...
	 if (( $rc == 0 )); then
		$(dirname $0)/narcoticsalerting_mail.sh
	 fi
 fi
 #======================================================#
